...
exports.choresRankView2 = function (action, targetChore, choreRankings) {
  ...
  const preferenceOptions = [
    { value: JSON.stringify({ strength: 0.7 }), text: common.blockPlaintext('a little') },
    { value: JSON.stringify({ strength: 1.0 }), text: common.blockPlaintext('a lot') },
  ];
  ...
  blocks.push(common.blockInput(
    `by ${(action === 'prioritize') ? 'deprioritizing' : 'prioritizing'}`,
    {
      action_id: 'chores',
  ...
  return {
    ...
    private_metadata: JSON.stringify({ targetChore, action }),
    title: TITLE,
    close: common.BACK,
    submit: common.NEXT,
    blocks,
